...
public class PDFGenerator {
    ...
    private static final Font PLAIN_FONT = new Font(Font.FontFamily.TIMES_ROMAN, 16, Font.NORMAL);
    ...
    private void addContent(Document pdf, int patientId, String name) throws DocumentException {
        ...
        paragraph.add(new Paragraph("Treatment details for " + name, TITLE_FONT));
    ...
    private static void createTreatmentEventsTable(
        ...
        Set<String> rowHeaders = Stream.of("Id", "Time", "Date", "Status", "Cancel Reason", "Treatment",
                "Type", "Dose", "Method", "Start Period", "End Period").collect(Collectors.toCollection(HashSet::new));
        ...
        for (TreatmentEventDTO tEvent : treatmentEventDTOS) {
            ...
            table.addCell(tEvent.getTreatmentEventTime());
            table.addCell(tEvent.getTreatmentEventDate());
            table.addCell(tEvent.getTreatmentEventStatus().toString());
            table.addCell(tEvent.getCancelReason());
            table.addCell(tEvent.getMedicineProcedureName());
            table.addCell(tEvent.getTreatmentType().toString());
            table.addCell(tEvent.getDose());
            table.addCell(tEvent.getAdministeringMedicationMethod());
            table.addCell(tEvent.getTreatmentPeriodStartDate());
            table.addCell(tEvent.getTreatmentPeriodEndDate());
            counter++;
        }
        pdf.add(table);
    }
    ...
    /**
     * Adds metadata to the PDF which can be viewed in your Adobe
     *
